...
Originally based on [EPNW/opus_dart](https://github.com/EPNW/opus_dart) (v3.0.1) and updated for Dart 3 and `wasm_ffi` 2.x compatibility.
...
`opus_codec_dart` has a pure-Dart test suite that runs without a physical device or a real libopus binary. All FFI calls are intercepted by [Mockito](https://pub.dev/packages/mockito) mocks, so tests are fast and fully hermetic.
